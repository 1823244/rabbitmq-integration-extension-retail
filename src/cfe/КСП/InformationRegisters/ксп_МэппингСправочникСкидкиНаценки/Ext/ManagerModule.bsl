...
Функция ПоМэппингу(GUID, ВнешняяСистема) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Рег.СкидкаНаценка КАК СкидкаНаценка
		|ИЗ
		|	РегистрСведений.ксп_МэппингСправочникСкидкиНаценки КАК Рег
		|ГДЕ
		|	Рег.GUID = &GUID
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Возврат ВыборкаДетальныеЗаписи.СкидкаНаценка;
	КонецЦикла;
...
Процедура ДобавитьГУИД(GUID, Наименование, ВнешняяСистема) Экспорт
	...
	НЗ = РегистрыСведений.ксп_МэппингСправочникСкидкиНаценки.СоздатьНаборЗаписей();
...
//	Параметр1 	- Тип1 - 
//
Процедура ДобавитьЗапись(GUID, Наименование, ВнешняяСистема, СкидкаНаценка) Экспорт
	...
	стрк.Наименование = Наименование;  
	стрк.ВнешняяСистема = ВнешняяСистема;	
	стрк.СкидкаНаценка = СкидкаНаценка;	
	НЗ.Записать();
