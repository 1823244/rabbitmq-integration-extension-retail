...
Процедура ДобавитьОтложенноеПроведение(ДанныеСсылка) Экспорт
	...
	стрк.СтатусПроведения = Перечисления.КСП_СтатусыОтложенногоПроведения.ПустаяСсылка();// по этому пустому значению будет выполняться отбор
...
Функция ОпределитьСтавкуНДСПоСправочникуЕРП(УзелСправочника) Экспорт
	...
	Если УзелСправочника.Свойство("ПеречислениеСтавкаНДС") Тогда
...
//
// Параметры:
//	мВнешняяСистема - строка - источник данных, напр. "upp"
//	ДанныеСсылка - ссылка - проверяемый объект
//	ТипДанных	- Строка - напр. "Справочник.Номенклатура"
//
// Возвращаемое значение:
//	Тип: Тип_значения
//
Функция ВерсияОбъектаВПриемнике(ВнешняяСистема, ДанныеСсылка, ТипДанных) Экспорт
	
	ПозицияТочки = СтрНайти(ТипДанных, ".");
	ТипОбъекта = Лев(ТипДанных, ПозицияТочки-1);
	ВидОбъекта = сред(ТипДанных, ПозицияТочки+1);
	Если Метаданные.РегистрыСведений.Найти("ксп_Версии_"+ТипОбъекта+"_"+ВидОбъекта) = Неопределено Тогда
		Возврат "";
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "Выбрать 
	|	РЕг.ВерсияДанных  как ВерсияДанных
	|из 
	|	РЕгистрсведений.ксп_Версии_Документ_ВнесениеДенежныхСредствВКассуККМ как Рег
	|Где 
	|	Рег.ВнешняяСИстема = &ВнешняяСИстема 
	|	и Рег.ДанныеСсылка = &ДанныеСсылка";
	
	Запрос.УстановитьПараметр("ВнешняяСИстема", ВнешняяСИстема);
	Запрос.УстановитьПараметр("ДанныеСсылка", ДанныеСсылка);
